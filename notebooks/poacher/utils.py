...
def frame_with_true_bboxes(img_path, labels):
    ...
    bboxes = postprocess_boxes(pred_bbox, frame_size, config.INPUT_SIZE, 0.3)
    bboxes = nms(bboxes, 0.45, method='nms')
    
    print('Num person on frame:', len(bboxes))
...
def predict_on_frame(frame, model, threshold=0.3, verbose=None):
    """
    """
    ...
    if verbose is not None:
        ...
        print(np.round(np.array([p for p in pred_bbox[:, 0:6] if p[...,4] > threshold])),2)
